...
function renderYear(year) {
  ...
  data[year].forEach(bearer => {
    ...
    card.className =
      'bg-white rounded-xl shadow-lg p-6 flex flex-col items-center text-center space-y-3 transform transition-transform duration-500 hover:-translate-y-2 hover:shadow-2xl';

    // Rectangular image with zoom effect on hover
   card.innerHTML = `
  <div class="overflow-hidden rounded-lg w-24 h-24 sm:w-28 sm:h-28 transform transition-transform duration-500 ease-in-out hover:scale-110 hover:rotate-1" data-aos="flip-left">
    <img src="${bearer.img}" alt="${bearer.name}"
